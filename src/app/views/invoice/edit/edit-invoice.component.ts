...
import { InvoiceService, CurrencyService, InvoiceDetailService, DescountReasonService } from '../../../service/invoice.service';
...
export class InvoiceEditComponent implements OnInit {
    ...
    validateRequidesFileds() {
        debugger
        let result = (!this.invoiceDetail.medicalServiceId || !this.invoiceDetail.price)
        return result;
    }
    ...
    saveHeader() {
        this.invoiceService.put(0, this.invoice).subscribe(response => {
    ...
    saveDetail() {
        if (!this.validateRequidesFileds) return this.alertService.error(this.translate.instant("vreqFiled"))
        this.invoiceDetail.discount=(this.invoiceDetail.qty * this.originalPrice)-(this.invoiceDetail.qty * this.invoiceDetail.price)
        this.invoiceDetail.price=this.originalPrice
        this.InvoiceDetailService.post(this.invoiceDetail).subscribe(response => {
    ...
    editDetail(item: invoiceDetail) {
        this.InvoiceDetailService.put(0, item).subscribe(response => {
    ...
    confirmDelete(id) {
        this.alertService.question(() => { this.delete(id) }, this.translate.instant('confirm'), this.translate.instant('sureTextRemove'))
      }
      delete(id) {
        this.InvoiceDetailService.delete(id).subscribe(response => {
