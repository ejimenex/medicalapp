...
export class QuestionComponent implements OnInit {
  ...
  ngOnInit() {
    this.filter.specifiedField = JSON.parse(
  ...
  getAll() {
    this.patientFormService
      .getByPatient(this.filter.specifiedField, this.patientId)
      .subscribe((response) => {
        this.data = response;
      });
  }
  update() {
    this.data.forEach((res) => {
      this.patientFormService.put(res.id, res).subscribe(
        (res) => {
          this.alert.successSWA(this.translate.instant("patientDataSaved"));
        },
        (error) => {
          this.alert.errorSWA("");
        }
