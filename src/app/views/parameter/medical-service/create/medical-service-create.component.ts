...
export class MedicalServiceCreateComponent implements OnInit {
    ...
    ngOnInit() {
        this.serviceModel.doctorGuid = JSON.parse(localStorage.getItem("currentUser")).doctorGuid;
    ...
    async getCurrencies() {
        this.currency = await this.medicalServiceService.getCurrency().toPromise() as any;
    }
    async getData() {
        this.serviceModel = await this.medicalServiceService.getById(this.id).toPromise() as any;
    }
    save(){
    ...
    add() {
        ...
        if (!this.serviceModel.name || !this.serviceModel.price || !this.serviceModel.currencyId)
            return this.alertService.error(this.translate.instant("vreqFiled"), 'Error')
        this.medicalServiceService.post(this.serviceModel).subscribe(response => {
    ...
    edit() {
        ...
        if (!this.serviceModel.name || !this.serviceModel.price || !this.serviceModel.currencyId)
            return this.alertService.error(this.translate.instant("vreqFiled"), 'Error')
        this.medicalServiceService.put(this.id,this.serviceModel).subscribe(response => {
